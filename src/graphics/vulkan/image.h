...
public:
    ...
    Image() = default;
    Image(Context* context, uint32_t width, uint32_t height, VkFormat format, VkImageUsageFlags usage, VkSampleCountFlagBits samples = VK_SAMPLE_COUNT_1_BIT) : context(context), format(format), width(width), height(height) {
    ...
    static std::unique_ptr<Image> LoadImage(Context* context, const std::string& filePath) {
        ...
        std::unique_ptr<Image> image = std::make_unique<Image>(context, width, height, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_SAMPLE_COUNT_1_BIT);
        context->StartAndSubmitCommandBuffer(context->graphics, [&image, &stagingBuffer](VkCommandBuffer commandBuffer) {
            ...
            image->CopyFrom(stagingBuffer, commandBuffer);
            image->TransitionLayout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, commandBuffer);
        });

        return image;
    }

    void CopyFrom(Buffer<uint8_t> &src, VkCommandBuffer commandBuffer = nullptr) {
        if (commandBuffer == nullptr) {
            context->StartAndSubmitCommandBuffer(context->graphics, [this, &src](VkCommandBuffer commandBuffer) {
                this->CopyFrom(src, commandBuffer);
            });
        } else {
    ...
    void TransitionLayout(VkImageLayout to, VkCommandBuffer commandBuffer = nullptr) {
        if (commandBuffer == nullptr) {
            context->StartAndSubmitCommandBuffer(context->graphics, [this, to](VkCommandBuffer buffer) {
                this->TransitionLayout(to, buffer);
            });
        } else {
